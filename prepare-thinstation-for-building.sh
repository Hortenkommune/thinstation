...
ahref=$(curl -s curl https://www.citrix.com/downloads/workspace-app/legacy-workspace-app-for-linux/workspace-app-for-linux-2112.html | grep linuxx64 | grep tar.gz | sed -r 's/^.+rel="([^"]+)".+$/\1/')
#IFS=' '
read -ra ADDR <<< "$ahref"
tarbLink="${ADDR/"//"/https://}"
