...
ahref=$(curl -s curl https://www.citrix.com/downloads/workspace-app/linux/workspace-app-for-linux-latest.html | grep linuxx64 | grep linuxx64 | grep tar.gz | sed -r 's/^.+rel="([^"]+)".+$/\1/')
#IFS=' '
read -ra ADDR <<< "$ahref"
tarbLink="${ADDR/"//"/https://}"
